...
class Tracker extends React.Component {
        ...
        request.get('https://raw.githubusercontent.com/lepelog/sslib/master/SS%20Rando%20Logic%20-%20Macros.yaml', (error, response, body) => {
            ...
            request.get('https://raw.githubusercontent.com/lepelog/sslib/master/SS%20Rando%20Logic%20-%20Item%20Location.yaml', (error, response, body) => {
                if (!error && response.statusCode === 200) {
                    ...
                    const locations = {};
                    let counter = 0;
                    const checksPerLocation = {};
                    const accessiblePerLocation = {};
                    const goddessCubes = [];
                    for (const location in doc) {
                        ...
                        if (location.includes('Goddess Chest')) {
                            const reqs = doc[location].Need.split(' & ');
                            const cubeReq = reqs.filter((req) => req.includes('Goddess Cube'))[0].trim();
                            const cube = {
                                localId: -1,
                                name: cubeReq.trim(),
                            ...
                            const id = goddessCubes.push(cube) - 1;
                            goddessCubes[id].localId = id;
                        }

                        const logicExpression = this.parseLogicExpression(doc[location].Need);
                        const finalRequirements = this.cleanUpLogicalString(
                        ...
                        const id = locations[group].push(newLocation) - 1;
    ...
    parseLogicExpressionToString(logicExpression, nestedLevel) {
        const requirements = logicExpression.slice();
    ...
    cleanUpLogicalString(expressions) {
        ...
        expressions.forEach((expression) => {
            ...
            if (expression.includes('Option ') && (!expression.includes(' and ') || !expression.includes('('))) {
                const optionSplit = expression.slice(8).split(/"/);
                if (this.state.options[optionSplit[0]] === (optionSplit[1].split('or')[0].trim() !== 'Disabled')) {
            ...
            if (expression.includes(' or ') && !(expression.includes(' and '))) {
                const split = expression.split(' or ');
    ...
    isMacro(macro) {
        ...
        const parsed = this.state.macros[macro];
        if (parsed === undefined) {
            return false;
        }
    ...
    checkAllRequirements() {
        for (const group in this.state.locations) {
    ...
    */
    ...
    meetsRequirement(requirement) {
        ...
        if (requirement.includes('Option ')) {
            const optionSplit = requirement.slice(8).split(/"/);
            // console.log(optionSplit)
            // console.log(optionSplit[1])
            return this.state.options[optionSplit[0]] === (optionSplit[1].trim() !== 'Disabled');
        }
        const macro = this.state.macros[requirement];
    ...
    handleLocationClick(group, location) {
        const newState = { ...this.state.locations }; // copy current state
        ...
        const add = newState[group][location].checked;
        ...
        const NewStateChecksPerLocation = { ...this.state.checksPerLocation };
        ...
        if (newState[group][location].inLogic) {
            const NewStateAccessiblePerLocation = { ...this.state.accessiblePerLocation };
    ...
    handleDungeonClick(dungeon) {
        const newRequiredDungeons = this.state.requiredDungeons.slice();
    ...
    updatePastMacro(dungeons) {
        ...
        const tmsLocation = updatedLocations['Sealed Grounds'][3];
